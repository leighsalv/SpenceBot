...
app.post('/webhook/', function(req, res) {
  ...
  for(let i=0; i < messaging_events.length; i++) {
    ...
    if(event.message && event.message.text) { //if there's a message & text..
      ...
      if(text == 'help' || 'HELP' || 'Help') { //user typed help -> bot tells instructions
      ...
      else if(text == 'total' || 'TOTAL' || 'Total') { //user typed total -> bot adds & replies total amount spent
      	totalAmount = 0;
      ...
      else if(text == 'clear' || 'CLEAR' || 'Clear') { //user typed clear -> bot clears spent log (array)
      	spent.length = 0;
